/*
** plan.c for rt in /home/descam_d/rendu/IGRAPH/MUL_2014_rtracer/scene_editor
** 
** Made by Corentin Descamps
** Login   <descam_d@epitech.net>
** 
** Started on  Fri Jun  5 13:24:04 2015 Corentin Descamps
** Last update Sun Jun  7 17:51:47 2015 Corentin Descamps
*/
...
static void		create_form_cons(GtkWidget **Label, GtkWidget **Entry)
{
  ...
  Label[1] = gtk_label_new("Brillance :");
  Entry[0] = gtk_entry_new();
  gtk_entry_set_max_length(GTK_ENTRY(Entry[0]), 3);
  Label[2] = gtk_label_new("Specularite :");
  Entry[1] = gtk_entry_new();
  gtk_entry_set_max_length(GTK_ENTRY(Entry[1]), 3);
  Label[3] = gtk_label_new("Transparence :");
  Entry[2] = gtk_entry_new();
  gtk_entry_set_max_length(GTK_ENTRY(Entry[2]), 3);
  Label[4] = gtk_label_new("Reflexion :");
...
{
  if (!(Widget->entry_cons = malloc(sizeof(GtkWidget*) * 4)))
    exit(1);
  if (!(Widget->label_cons = malloc(sizeof(GtkWidget*) * 5)))
    exit(1);
}

void		form_plan(GtkWidget *Window)
{
  t_wid		*Widget;
  GtkWidget	*Save;
  GtkWidget	*Grid;
  ...
  g_signal_connect(G_OBJECT(Save), "clicked", G_CALLBACK(save_plan), Widget);
}

void		add_plan(GtkWidget *Widget, gpointer data)
{
  GtkWidget	*Window;
  ...
  gtk_window_set_title(GTK_WINDOW(Window), "New Plan");
